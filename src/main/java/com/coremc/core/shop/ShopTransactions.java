...
public final class ShopTransactions {
    ...
    public static int countSellable(final ItemStack[] contents, final Material material) {
        ...
        for (final ItemStack stack : contents) {
            if (stack != null && stack.getType() == material && !isCustomItem(stack)) {
     ...
     * relics, spawner items...). Those are progression items, never raw
     * shop goods, so they must not be sellable as their base material —
     * the /sell window returns them instead of paying for them.
     */
