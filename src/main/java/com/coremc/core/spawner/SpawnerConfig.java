...
public final class SpawnerConfig {
    ...
    private void parseGroups(final YamlConfiguration yaml, final List<String> problems) {
        ...
        for (final String groupId : root.getKeys(false)) {
            ...
            for (final String mobId : mobsSection.getKeys(false)) {
                final SpawnerMob mob = parseMob(groupId, mobsSection.getConfigurationSection(mobId), problems);
                if (mob != null) {
                    mobs.add(mob);
                }
            }
    ...
    private SpawnerMob parseMob(final String groupId, final ConfigurationSection section,
        ...
        final Map<SpawnerVariant, SpawnerUpgradeCost> upgrades =
                new EnumMap<>(SpawnerVariant.class);
        final ConfigurationSection upgradesSection = section.getConfigurationSection("upgrades");
        if (upgradesSection != null) {
            for (final String key : upgradesSection.getKeys(false)) {
                final SpawnerVariant variant = SpawnerVariant.of(key);
                if (variant == null || variant == SpawnerVariant.NORMAL) {
                    problems.add("group '" + groupId + "' mob '" + mobId + "': upgrade '" + key
                            + "' is not a variant above normal");
                    continue;
                }
                final ConfigurationSection cost = upgradesSection.getConfigurationSection(key);
                if (cost == null) {
                    problems.add("group '" + groupId + "' mob '" + mobId + "': upgrade '" + key
                            + "' is not a mapping");
                    continue;
                }
                upgrades.put(variant, new SpawnerUpgradeCost(
        ...
        for (final SpawnerVariant variant : SpawnerVariant.values()) {
            if (variant != SpawnerVariant.NORMAL && !upgrades.containsKey(variant)) {
