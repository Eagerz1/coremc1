...
// config.yml: make spawner-kill rewards deterministic (chance 1.0) so the
// spawner economy path is asserted on every run. All mechanics and the
// reward amounts themselves stay at stock values.
const config = yamlLoad(fs.readFileSync(path.join(SRC, 'config.yml'), 'utf8'))
if (!config.spawners || !config.spawners['kill-rewards']) {
  throw new Error('stock config.yml has no spawners.kill-rewards table')
}
config.spawners['kill-rewards'].chance = 1.0
fs.writeFileSync(path.join(OUT, 'config.yml'), yamlDump(config))
